...
void k_startup()
{
	...
	arch_register_interrupt_handler(INT_MEM_FAULT, k_memory_fault, NULL,7); //DODAN
	arch_register_interrupt_handler(INT_UNDEF_FAULT, k_memory_fault, NULL,8); //DODAN
	...
	//keyboard();
	//timer();
	 segm_fault();  //OVO JE NJIHOVA VERZIJA, VIDI PRVO NJU!!
